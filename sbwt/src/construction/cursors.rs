...
use std::io::SeekFrom;
use std::cmp::min;
use crate::{kmer::LongKmer, util::binary_search_leftmost_that_fulfills_pred};
...
-> Vec<DummyNodeMerger<BufReader<File>, B>>{
    ...
    for c in 0..(sigma as u8){
        log::info!("Searching character {}", c);
...
mod tests{
    ...
    fn test_init_char_cursors(){
        ...
        let (nondummy_file, mut nondummy_writer) = create_temp_file(std::path::Path::new("temp"));
        let (dummy_file, mut dummy_writer) = create_temp_file(std::path::Path::new("temp"));

        for kmer in nondummies.iter(){
            kmer.serialize(&mut nondummy_writer).unwrap();
        }
        for (kmer, len) in dummies.iter(){
            kmer.serialize(&mut dummy_writer).unwrap();
            let len_byte = *len as u8;
            dummy_writer.write_all(&[len_byte]).unwrap();
        }

        // Flush
        drop(dummy_writer);
        drop(nondummy_writer);

        let char_cursors = init_char_cursors(dummy_file.path(), nondummy_file.path(), 4, 4);
